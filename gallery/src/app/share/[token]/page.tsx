...
export default function SharePage() {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center" style={{ background: 'var(--background)' }}>
        <div className="flex flex-col items-center gap-4 animate-fade-in">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'var(--accent-gradient)' }}>
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--background)' }}>
        <div className="text-center animate-fade-in max-w-sm">
          ...
          <p className="text-sm" style={{ color: 'var(--foreground-secondary)' }}>링크가 만료되었거나 잘못된 링크입니다.</p>
        </div>
      </main>
    )
  }
  ...
  const isVideo = photo.name.match(/\.(mp4|webm|mov|avi|mkv)$/i)

  return (
    <main className="min-h-screen flex flex-col safe-area-top safe-area-bottom" style={{ background: 'var(--background)' }}>
      {/* 헤더 */}
      <header className="header">
        <div className="header-content">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center" style={{ background: 'var(--accent-primary)', boxShadow: 'var(--shadow-md)' }}>
              <svg className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: 'var(--accent-text)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span className="text-base sm:text-lg font-bold">Cloody</span>
          </div>
          <button
      ...
      <div className="flex-1 flex items-center justify-center p-3 sm:p-4 md:p-6">
        <div className="max-w-4xl w-full animate-fade-in">
          <div className="rounded-xl sm:rounded-2xl overflow-hidden" style={{ background: 'var(--background-secondary)' }}>
            {isVideo ? (
              <video
                src={imageUrl}
                controls
          ...
          <div className="mt-4 sm:mt-6 text-center">
            <p className="font-medium truncate text-sm sm:text-base px-4">{photo.name}</p>
      ...
      <footer className="py-4 sm:py-6 text-center">
        <p className="text-xs sm:text-sm" style={{ color: 'var(--foreground-muted)' }}>
          Powered by <span className="gradient-text font-medium">Cloody</span>
        </p>
      </footer>
    </main>
  )
